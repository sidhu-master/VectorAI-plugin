...
import type { OpeningAngleFact, OpeningAngleLayout } from './types';
...
export function layoutOpeningAngles(input: {
  ...
  const facts = [...input.facts].sort((left, right) => {
    ...
    return (left.vertex[0] - right.vertex[0]) * openingSide(left.vertex[0], bounds)
      || left.value - right.value || left.key.localeCompare(right.key);
  });
  for (const fact of facts) {
    const side = openingSide(fact.vertex[0], bounds);
    const lane = laneBySide.get(side) ?? 0;
    laneBySide.set(side, lane + 1);
    const distToEdge = side === -1 ? fact.vertex[0] - bounds.minX : bounds.maxX - fact.vertex[0];
...
function openingSide(x: number, bounds: { minX: number; maxX: number }): -1 | 1 { return x <= (bounds.minX + bounds.maxX) / 2 ? -1 : 1; }
