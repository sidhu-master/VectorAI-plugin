...
const revision: PartitionRevision = {
  ...
  semanticGroups: [], evidence: [], diagnostics: [],
};
...
describe('ConfirmedPartitionInspector', () => {
  it('shows the confirmed structure and explicitly reopens it for editing', async () => {
    ...
    const renderer = TestRenderer.create(<ConfirmedPartitionInspector revision={revision} busy={false} onReopen={reopen} />);
    const markup = renderToStaticMarkup(<ConfirmedPartitionInspector revision={revision} busy={false} onReopen={reopen} />);
    expect(markup).toContain('partition-r1');
    expect(markup).toContain('左轴承位');
    expect(markup).toContain('主轮齿轴段');
